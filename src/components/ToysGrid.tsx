...
export const ToysGrid = () => {
  ...
  const toyWords = ['ballon', 'poupée', 'jouer', 'courir', 'sauter'];

  useEffect(() => {
    loadToysVideos();
  }, []);

  const loadToysVideos = async () => {
    setLoading(true);
    
    const toysWithVideos: Toy[] = await Promise.all(
      toyWords.map(async (word) => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        ...
        <span className="ml-2">Chargement des jeux...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-2xl font-bold mb-4">Jeux et Actions en LSFB</h3>
